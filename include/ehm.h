...
/**
 * @brief The exception handing frame.
 * The jmp_buf field is first to avoid padding.
 */
struct ehm_frame
{
    jmp_buf env;
 ...
 * TRY, CATCH, or FINALLY block. If any of the statements must return, then
...
ehm_throw(const ehm_exception *e, const ch8 *file, umm line, const ch8 *func);
...
#else   // NDEBUG
// Throw an exception that can only be caught by a EHM_CATCH_ALL block.
